  ...
  return (
    ...
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg '>
        <p className='text-2xl font-semibold '>{state === 'Sign Up'? 'Create User Account': ' User Login'}</p>
        ...
        <button>Go back to Home: <span className='text-[#5F6FFF] underline cursor-pointer' onClick={()=>navigate("/")}>Click here</span></button>
      </div>
    </form> 
    </>
  )
